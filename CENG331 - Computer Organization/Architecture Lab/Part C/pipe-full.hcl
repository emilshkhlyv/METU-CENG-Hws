...
##
## Stages:	Operations:				## Comments:
##	
## Fetch: 	icode:ifun <- M1[PC]  			## icode <- C; ifun <- 0;
##		rA:rB <- M1[PC+1]			## rA is F, it means register
##		valC <- M8[PC+2]			## valC is V
##		valP <- PC+10				## size of instruction is 10 bytes
##
## Decode:	valB <- R[rB]				## read value of rB
##
## Execute:	valE <- valB + valC			## aluA <- valC; aluB <- valB
##		set Condition codes (ZF, OF, SF)	## Set condition flags	
##
## Memory:	#No operation in Memory Stage
##
## Write Back:	R[rB] <- valE				## Write the value of valE to rB
##
## PC: 		PC <- valP				## set the value of PC to valP
## 
...
# ADDED here, because it uses rB
bool need_regids =
	f_icode in { IIADDQ, IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, 
		     IIRMOVQ, IRMMOVQ, IMRMOVQ };

# Does fetched instruction require a constant word?
# ADDED here. because it gets valC
bool need_valC =
...
# ADDED here, because source of valB is D_rb
word d_srcB = [
...
# ADDED here, because of writing of E to D_rB
word d_dstE = [
